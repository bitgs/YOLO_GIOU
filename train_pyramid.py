...
import args_pyramid as args

from utils.data_utils_pyramid import get_batch_data
...
from model_pyramid import yolov3
...
train_dataset = train_dataset.map(
    lambda x: tf.py_func(get_batch_data,
                         ...
                         Tout=[tf.int64, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32, tf.float32]),
...
image_ids, image, image_half, image_quar, y_true_13, y_true_26, y_true_52 = iterator.get_next()
y_true = [y_true_13, y_true_26, y_true_52]

# tf.data pipeline will lose the data `static` shape, so we need to set it manually
image_ids.set_shape([None])
image.set_shape([None, None, None, 3])
image_half.set_shape([None, None, None, 3])
image_quar.set_shape([None, None, None, 3])
for y in y_true:
...
with tf.variable_scope('yolov3'):
    route_1, route_2, route_3 = yolo_model.forward_body(image, image_half, image_quar,
                                                        is_training=is_training, reuse=tf.AUTO_REUSE)
    pred_feature_maps = yolo_model.forward_head(route_1, route_2, route_3, is_training=is_training)
# =============================================================================
...
with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        for i in trange(args.train_batch_num):
            _, summary, __y_pred, __y_true, __loss, __global_step, __lr,__image,__image_half,__image_quar,\
            __route_2, __route_3 = sess.run(
                [train_op, merged, y_pred, y_true, loss, global_step, learning_rate, image, image_half, image_quar,
                 route_2, route_3],
                feed_dict={is_training: True})
            temp1 = __image[0]
            temp2 = __image_half[0]
            temp3 = __image_quar[0]
            temp4 = __route_2[0]
            temp5 = __route_3[0]

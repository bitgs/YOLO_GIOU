...
restore_path = './data/darknet_weights/yolov3.ckpt'#'/home/yuan/Downloads/inception_resnet_v2_2016_08_30.ckpt'  # The path of the weights to restore.
...
anchor_path = './data/coco_anchors_pyramid.txt'  # The path of the anchor txt file.
class_name_path = './data/voc.names'  # The path of the class names.

### Training releated numbers
batch_size = 4
img_size = [1024, 1024]#32*32#[416, 416]  # Images will be resized to `img_size` and fed to the network, size format: [width, height]
...
### tf.data parameters
num_threads = 6  # Number of threads for image processing used in tf.data pipeline.
...
lr_decay_factor = 0.95  # The learning rate decay factor. Used when chosen `exponential` lr_type.
lr_lower_bound = 1e-6  # The minimum learning rate.
# only used in piecewise lr type
pw_boundaries = [20, 35]  # epoch based boundaries
...
restore_include = ['yolov3/darknet53_body/']
restore_exclude = None#['yolov3_head']#['yolov3/yolov3_head/Conv_14', 'yolov3/yolov3_head/Conv_6', 'yolov3/yolov3_head/Conv_22']
...
warm_up_epoch = 0  # Warm up training epoches. Set to a larger value if gradient explodes.

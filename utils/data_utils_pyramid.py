...
# =============================================================================
    y_true_13 = np.zeros((img_size[1] // 128, img_size[0] // 128, 3, 6 + class_num), np.float32)
    y_true_26 = np.zeros((img_size[1] // 64,  img_size[0] // 64,  3, 6 + class_num), np.float32)
    y_true_52 = np.zeros((img_size[1] // 32,  img_size[0] // 32,  3, 6 + class_num), np.float32)
...
def parse_data(line, class_num, img_size, anchors, mode, letterbox_resize):
    ...
    if not isinstance(line, list):
        ...
        #TODO
        #cannot use mix up for image pyramid now
        # the mix up case
...
# =============================================================================
    ...
    img = img / 255.
    img_half = cv2.resize(img, (int(img_size[1]/2), int(img_size[0]/2)))
    img_quar = cv2.resize(img_half, (int(img_size[1]/4), int(img_size[0]/4)))

    y_true_13, y_true_26, y_true_52 = process_box(boxes, labels, img_size, class_num, anchors)

    return img_idx, img, img_half, img_quar, y_true_13, y_true_26, y_true_52
...
def get_batch_data(batch_line, class_num, img_size, anchors, mode, multi_scale=False, mix_up=False, letterbox_resize=True, interval=10):
    ...
    for line in batch_line:
        img_idx, img, img_half, img_quar, y_true_13, y_true_26, y_true_52 = parse_data(line, class_num, img_size, anchors, mode, letterbox_resize)

        img_idx_batch.append(img_idx)
        img_batch.append(img)
        img_half_batch.append(img_half)
        img_quar_batch.append(img_quar)
    ...
    img_half_batch, img_quar_batch = np.asarray(img_half_batch), np.asarray(img_quar_batch)
    return img_idx_batch, img_batch, img_half_batch, img_quar_batch, y_true_13_batch, y_true_26_batch, y_true_52_batch
